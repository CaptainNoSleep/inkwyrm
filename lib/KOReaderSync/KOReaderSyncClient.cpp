...
// Perform the request, retrying the whole exchange on TRANSPORT failure (a
// dropped TLS handshake packet on a marginal 2.4GHz link surfaces as a non-OK
// esp_err_t, NOT an HTTP status). esp_http_client_perform reopens the
// connection on each call. HTTP-level results (200/401/5xx) are returned as-is;
// only network/transport errors retry. Bounded so a truly-down server fails
// fast enough to keep the sync UI responsive.
esp_err_t performWithRetry(esp_http_client_handle_t client) {
  ...
  for (int attempt = 1; attempt <= MAX_ATTEMPTS; ++attempt) {
    ...
    if (err == ESP_OK) {
      if (attempt > 1) LOG_DBG("KOSync", "Request succeeded on attempt %d", attempt);
      return ESP_OK;
    }
    LOG_DBG("KOSync", "Transport error (attempt %d/%d): %d", attempt, MAX_ATTEMPTS, err);
...
KOReaderSyncClient::Error KOReaderSyncClient::authenticate() {
  ...
  esp_err_t err = performWithRetry(client);
...
KOReaderSyncClient::Error KOReaderSyncClient::getProgress(const std::string& documentHash,
  ...
  if (httpCode == 401) return AUTH_FAILED;
  // the KOSync server (kosync-dotnet-compatible) reports "no saved progress
  // for this document" as HTTP 502 with a JSON body, NOT 404. Without this, a
  // brand-new book (nothing ever pushed) surfaces as "sync failed" instead of
  // the correct "no remote progress — upload?" path. Treat both as NOT_FOUND.
