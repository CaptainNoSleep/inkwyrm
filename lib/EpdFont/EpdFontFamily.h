...
class EpdFontFamily {
 public:
  ...
  // Bits 0-1 select the font variant (BOLD/ITALIC); bits 2-7 are decoration/positioning overlays
  ...
  enum Style : uint8_t {
    ...
    DROP_CAP = 64,      // enlarged initial letter rendered with a larger matching font size
    SMALL_CAPS = 128,   // uppercase text rendered with the next-smaller matching font size
  };
