...
struct BlockStyle {
  ...
  // Typography
  CssLength lineHeight;  // CSS line-height; unitless multipliers are stored as raw values
  bool lineHeightDefined = false;
  bool lineHeightIsMultiplier = true;
  bool fontVariantSmallCaps = false;
  float initialLetter = 0.0f;
  ...
  [[nodiscard]] BlockStyle getCombinedBlockStyle(const BlockStyle& child, CombineAxis axis) const {
    ...
    if (!child.lineHeightDefined && lineHeightDefined) {
      ...
      result.lineHeightIsMultiplier = lineHeightIsMultiplier;
    }
    if (!child.fontVariantSmallCaps && fontVariantSmallCaps) {
      result.fontVariantSmallCaps = true;
    }
  ...
  static BlockStyle fromCssStyle(const CssStyle& cssStyle, const float emSize, const CssTextAlign paragraphAlignment,
    ...
    if (cssStyle.hasLineHeight()) {
      if (cssStyle.lineHeightIsMultiplier) {
        blockStyle.lineHeight = cssStyle.lineHeight;
        blockStyle.lineHeight.unit = CssUnit::Pixels;
      } else {
        blockStyle.lineHeight = cssStyle.lineHeight;
        if (blockStyle.lineHeight.unit == CssUnit::Percent) {
          blockStyle.lineHeight.value = blockStyle.lineHeight.value * emSize / 100.0f;
          blockStyle.lineHeight.unit = CssUnit::Pixels;
        } else {
          blockStyle.lineHeight.value = blockStyle.lineHeight.toPixels(emSize);
          blockStyle.lineHeight.unit = CssUnit::Pixels;
        }
      }
      blockStyle.lineHeightDefined = true;
      blockStyle.lineHeightIsMultiplier = cssStyle.lineHeightIsMultiplier;
    }
    blockStyle.fontVariantSmallCaps = cssStyle.fontVariantSmallCaps;
    blockStyle.initialLetter = cssStyle.initialLetter;
