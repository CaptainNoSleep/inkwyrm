...
static int resolveLineHeightPx(const GfxRenderer& renderer, const BlockStyle& blockStyle, const int fontId,
                               const float lineCompression) {
  const int baseLineHeight = static_cast<int>(renderer.getLineHeight(fontId) * lineCompression);
  if (!blockStyle.lineHeightDefined) return baseLineHeight;
  if (blockStyle.lineHeightIsMultiplier) return static_cast<int>(baseLineHeight * blockStyle.lineHeight.value);
  return static_cast<int>(blockStyle.lineHeight.value + 0.5f);
}
...
void ChapterHtmlSlimParser::updateEffectiveInlineStyle() {
  ...
  effectiveSup = false;
  effectiveSub = false;
  effectiveSmallCaps = currentCssStyle.hasFontVariantSmallCaps();
...
void ChapterHtmlSlimParser::flushPartWordBuffer() {
  ...
  if (currentTextBlock && currentTextBlock->getBlockStyle().initialLetter > 1.0f && currentTextBlock->size() == 0) {
    fontStyle = static_cast<EpdFontFamily::Style>(fontStyle | EpdFontFamily::DROP_CAP);
  }
...
void ChapterHtmlSlimParser::emitHorizontalRule(const BlockStyle& blockStyle) {
  ...
  const int16_t lineHeight = static_cast<int16_t>(resolveLineHeightPx(renderer, blockStyle, fontId, lineCompression));
...
void XMLCALL ChapterHtmlSlimParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (matches(name, HEADER_TAGS, std::size(HEADER_TAGS))) {
    ...
    if (cssStyle.hasFontVariantSmallCaps()) {
    ...
    if (cssStyle.hasFontWeight() || cssStyle.hasFontStyle() || cssStyle.hasTextDecoration() ||
        cssStyle.hasDirection() || cssStyle.hasVerticalAlign() || cssStyle.hasFontVariantSmallCaps()) {
      // Flush buffer before style change so preceding text gets current style
      if (self->partWordBufferIndex > 0) {
        self->flushPartWordBuffer();
        self->nextWordContinues = true;
...
void ChapterHtmlSlimParser::addLineToPage(std::shared_ptr<TextBlock> line) {
  const int lineHeight = resolveLineHeightPx(renderer, line->getBlockStyle(), fontId, lineCompression);
...
void ChapterHtmlSlimParser::makePages() {
  ...
  const int lineHeight = resolveLineHeightPx(renderer, blockStyle, fontId, lineCompression);
