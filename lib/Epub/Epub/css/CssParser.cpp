...
void CssParser::parseDeclarationIntoStyle(std::string_view decl, CssStyle& style) {
  ...
  if (iequalsAscii(name, "text-align")) {
    ...
    if (tryParseNumber(stripped, intValue)) {
      style.lineHeight = CssLength(static_cast<float>(intValue));
      style.lineHeightDefined = true;
      style.lineHeightIsMultiplier = true;
      style.defined.lineHeight = 1;
    } else if (tryParseNumber(stripped, floatValue)) {
      style.lineHeight = CssLength(floatValue);
      style.lineHeightDefined = true;
      style.lineHeightIsMultiplier = true;
      style.defined.lineHeight = 1;
    } else {
      CssLength len;
      if (tryInterpretLength(stripped, len)) {
    ...
    if (icontainsAscii(value, "small-caps")) {
      style.fontVariantSmallCaps = true;
      style.defined.fontVariantSmallCaps = 1;
    }
  } else if (iequalsAscii(name, "initial-letter")) {
    const std::string_view stripped = stripTrailingImportant(value);
    const size_t spacePos = stripped.find_first_of(" \t\r\n\f");
    const std::string_view firstToken = trimCssWhitespace(stripped.substr(0, spacePos));
    int initialLetterLines = 0;
    if (!firstToken.empty() && tryParseNumber(firstToken, initialLetterLines) && initialLetterLines > 1) {
      style.initialLetter = static_cast<float>(initialLetterLines);
...
bool CssParser::saveToCache() const {
  ...
  for (const auto& pair : rulesBySelector_) {
    ...
    file.write(reinterpret_cast<const uint8_t*>(&style.lineHeight.value), sizeof(style.lineHeight.value));
    file.write(static_cast<uint8_t>(style.lineHeight.unit));
    file.write(static_cast<uint8_t>(style.lineHeightDefined ? 1 : 0));
    file.write(static_cast<uint8_t>(style.lineHeightIsMultiplier ? 1 : 0));
    file.write(static_cast<uint8_t>(style.fontVariantSmallCaps ? 1 : 0));
    file.write(reinterpret_cast<const uint8_t*>(&style.initialLetter), sizeof(style.initialLetter));
    ...
    if (style.defined.lineHeight) definedBits |= 1 << 18;
    if (style.defined.fontVariantSmallCaps) definedBits |= 1 << 19;
    if (style.defined.initialLetter) definedBits |= 1 << 20;
...
bool CssParser::loadFromCache() {
  ...
  constexpr size_t CSS_FIXED_STYLE_BYTES =
      5 * sizeof(uint8_t) + CSS_LENGTH_BYTES + 3 * sizeof(uint8_t) + sizeof(float) +
      (CSS_LENGTH_FIELD_COUNT * CSS_LENGTH_BYTES) + 2 * sizeof(uint8_t) + sizeof(uint32_t);
  ...
  for (uint16_t i = 0; i < ruleCount; ++i) {
    ...
    if (file.read(&style.lineHeight.value, sizeof(style.lineHeight.value)) != sizeof(style.lineHeight.value)) {
      rulesBySelector_.clear();
      return false;
    }
    uint8_t lineHeightUnit = 0;
    if (file.read(&lineHeightUnit, 1) != 1) {
      rulesBySelector_.clear();
      return false;
    }
    style.lineHeight.unit = static_cast<CssUnit>(lineHeightUnit);

    uint8_t boolVal = 0;
    if (file.read(&boolVal, 1) != 1) {
      rulesBySelector_.clear();
      return false;
    }
    style.lineHeightDefined = boolVal != 0;
    if (file.read(&boolVal, 1) != 1) {
      rulesBySelector_.clear();
      return false;
    }
    style.lineHeightIsMultiplier = boolVal != 0;
    if (file.read(&boolVal, 1) != 1) {
      rulesBySelector_.clear();
      return false;
    }
    style.fontVariantSmallCaps = boolVal != 0;
    if (file.read(&style.initialLetter, sizeof(style.initialLetter)) != sizeof(style.initialLetter)) {
      rulesBySelector_.clear();
      return false;
    }
    ...
    style.defined.lineHeight = (definedBits & 1 << 18) != 0;
    style.defined.fontVariantSmallCaps = (definedBits & 1 << 19) != 0;
    style.defined.initialLetter = (definedBits & 1 << 20) != 0;
