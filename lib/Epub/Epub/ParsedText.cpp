...
uint16_t measureWordWidth(const GfxRenderer& renderer, const int fontId, const std::string& word,
  ...
  const int typographyFontId = TypographyUtils::resolveTypographyFontId(fontId, style);
  if (word.size() == 1 && word[0] == ' ' && !appendHyphen) {
    return renderer.getSpaceWidth(typographyFontId, style);
  }
  const bool hasSoftHyphen = containsSoftHyphen(word);
  if (!hasSoftHyphen && !appendHyphen) {
    return renderer.getTextAdvanceX(typographyFontId, word.c_str(), style);
  }
  ...
  return renderer.getTextAdvanceX(typographyFontId, sanitized.c_str(), style);
}
...
void ParsedText::addWord(std::string word, const EpdFontFamily::Style fontStyle, const bool underline,
  ...
  if ((baseStyle & EpdFontFamily::SMALL_CAPS) != 0) {
    TypographyUtils::toUpperAsciiInPlace(word);
  }
  if (underline) {
