...
Welcome to the **InkWyrm** firmware. This guide outlines the hardware controls, navigation, and reading features of the device.

- [InkWyrm User Guide](#inkwyrm-user-guide)
...
InkWyrm supports sending books from Calibre using the CrossPoint Reader device plugin.
...
- **Sleep Screen**: Which sleep screen to display when the device sleeps:
  ...
  - "Dark" (default) - The default dark InkWyrm logo sleep screen
...
- **UI Theme**: Set which UI theme to use:
  ...
  - "Classic" - The original InkWyrm theme
  - "Lyra" - The new theme for InkWyrm featuring rounded elements and menu icons
...
- **Check for updates**: Check for InkWyrm firmware updates over Wi-Fi. Firmware can also be updated without a USB connection by placing a `firmware.bin` file on the SD card.

- **Language**: Set the UI language. InkWyrm supports 24 languages: English, Spanish, French, German, Czech, Brazilian Portuguese, Russian, Swedish, Romanian, Catalan, Ukrainian, Belarusian, Italian, Polish, Finnish, Danish, Dutch, Turkish, Kazakh, Hungarian, Lithuanian, Slovenian, Valencian, and Hebrew.
...
InkWyrm supports saving multiple OPDS servers and switching between them when browsing catalogs.
...
InkWyrm can sync reading progress with KOReader-compatible sync servers.
...
2. On each InkWyrm device:
   
   - Go to **Settings -> System -> KOReader Sync**.
   
   - Set **Username** and **Password** (enter the plain password; InkWyrm computes MD5 internally, and use the same values on all devices).
...
3. Register a user once.
   InkWyrm authenticates against KOReader Sync (`koreader/kosync`) using an MD5 key, so register using the MD5 of your password:
...
| Mode               | Behavior                                                                                                                     |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| **Dark** (default) | The InkWyrm logo on a dark background.                                                                                    |
| **Light**          | The InkWyrm logo on a white background.                                                                                   |
...
InkWyrm supports loading additional fonts from the SD card, extending beyond the two built-in families (Noto Serif, Noto Sans). Custom fonts can include extended Unicode coverage, enabling CJK (Chinese, Japanese, Korean) and other scripts.
...
InkWyrm renders text using the following Unicode character blocks, enabling support for a wide range of languages:
...
If an issue or crash is encountered while using InkWyrm, feel free to raise an issue ticket and attach the logs.

**Crash reports on SD card:** After a crash, InkWyrm automatically saves a crash report to the SD card (no USB connection needed). Check the root of the SD card for a crash log file and include it with any bug report.
