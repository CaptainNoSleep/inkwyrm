...
#include "activities/reader/HighlightStore.h"
...
bool JsonSettingsIO::saveState(const CrossPointState& s, const char* path) {
  ...
  doc["koSyncAutoPullResumePath"] = s.koSyncAutoPullResumePath;
...
bool JsonSettingsIO::loadState(CrossPointState& s, const char* json) {
  ...
  s.koSyncAutoPullResumePath = doc["koSyncAutoPullResumePath"] | std::string("");
  return true;
}
...
bool JsonSettingsIO::saveSettings(const CrossPointSettings& s, const char* path) {
  ...
  doc["lastKoSyncEpoch"] = s.lastKoSyncEpoch;
...
bool JsonSettingsIO::loadSettings(CrossPointSettings& s, const char* json, bool* needsResave) {
  ...
  s.dailyGoalTarget = clamp(doc["dailyGoalTarget"] | s.dailyGoalTarget,
                            CrossPointSettings::DAILY_GOAL_TARGET_COUNT, s.dailyGoalTarget);
  s.readingStatsAutoBackup = clamp(doc["readingStatsAutoBackup"] | s.readingStatsAutoBackup,
                                   CrossPointSettings::READING_STATS_AUTOBACKUP_COUNT, s.readingStatsAutoBackup);
  s.lastKoSyncEpoch = doc["lastKoSyncEpoch"] | static_cast<uint32_t>(0);
...
bool JsonSettingsIO::saveHighlights(const HighlightStore& store, const char* path) {
  JsonDocument doc;
  doc["formatVersion"] = HighlightStore::FILE_VERSION;
  doc["bookId"] = store.bookId;
  JsonArray arr = doc["highlights"].to<JsonArray>();
  for (const auto& highlight : store.getAll()) {
    JsonObject obj = arr.add<JsonObject>();
    obj["id"] = highlight.id;
    obj["bookId"] = highlight.bookId;
    obj["spineIndex"] = highlight.spineIndex;
    obj["pageNumber"] = highlight.pageNumber;
    obj["anchorWordIndex"] = highlight.anchorWordIndex;
    obj["focusWordIndex"] = highlight.focusWordIndex;
    obj["selectedText"] = highlight.selectedText;
    obj["createdAt"] = highlight.createdAt;
    obj["note"] = highlight.note;
  }

  return saveJsonDocumentToFile("HLT", path, doc);
}

bool JsonSettingsIO::loadHighlights(HighlightStore& store, const char* json) {
  JsonDocument doc;
  auto error = deserializeJson(doc, json);
  if (error) {
    LOG_ERR("HLT", "JSON parse error: %s", error.c_str());
    CPR_VCODEX_LOG_EVENT("HLT", std::string("Highlights JSON parse error: ") + error.c_str());
    return false;
  }

  store.highlights.clear();
  const uint32_t formatVersion = doc["formatVersion"] | static_cast<uint32_t>(1);
  if (formatVersion > HighlightStore::FILE_VERSION) {
    LOG_ERR("HLT", "Unsupported highlights format version: %u", static_cast<unsigned>(formatVersion));
    return false;
  }
  const std::string fileBookId = doc["bookId"] | std::string("");
  if (!store.bookId.empty() && !fileBookId.empty() && store.bookId != fileBookId) {
    LOG_ERR("HLT", "Book id mismatch while loading highlights: expected %s got %s", store.bookId.c_str(),
            fileBookId.c_str());
    return false;
  }
  if (store.bookId.empty() && !fileBookId.empty()) {
    store.bookId = fileBookId;
  }

  JsonArray arr = doc["highlights"].as<JsonArray>();
  for (JsonObject obj : arr) {
    HighlightStore::HighlightEntry highlight;
    highlight.id = obj["id"] | std::string("");
    highlight.bookId = obj["bookId"] | store.bookId;
    highlight.spineIndex = obj["spineIndex"] | static_cast<uint16_t>(0);
    highlight.pageNumber = obj["pageNumber"] | static_cast<uint16_t>(0);
    highlight.anchorWordIndex = obj["anchorWordIndex"] | static_cast<uint16_t>(0);
    highlight.focusWordIndex = obj["focusWordIndex"] | static_cast<uint16_t>(0);
    highlight.selectedText = obj["selectedText"] | std::string("");
    highlight.createdAt = obj["createdAt"] | static_cast<uint32_t>(0);
    highlight.note = obj["note"] | std::string("");
    if (highlight.id.empty()) {
      highlight.id = store.makeHighlightId(highlight.createdAt ? highlight.createdAt : static_cast<uint32_t>(time(nullptr)));
    }
    if (highlight.bookId.empty()) {
      highlight.bookId = store.bookId;
    }
    store.highlights.push_back(std::move(highlight));
  }

  return true;
}

bool JsonSettingsIO::loadHighlightsFromFile(HighlightStore& store, const char* path) {
  String json = Storage.readFile(path);
  if (json.isEmpty()) {
    return false;
  }
  return loadHighlights(store, json.c_str());
}
