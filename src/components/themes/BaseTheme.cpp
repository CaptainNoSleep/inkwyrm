...
constexpr int compactInfoSlotGap = 6;
constexpr int compactInfoMinTitleWidth = 56;
constexpr size_t compactInfoTitleBufferSize = 192;

struct CompactInfoBarLayout {
...
void truncateCompactInfoTitle(const GfxRenderer& renderer, const std::string& title, const int maxWidth,
                              char (&buffer)[compactInfoTitleBufferSize]) {
  buffer[0] = '\0';
  if (title.empty() || maxWidth <= 0) return;

  if (title.size() < sizeof(buffer) && renderer.getTextWidth(SMALL_FONT_ID, title.c_str()) <= maxWidth) {
    std::memcpy(buffer, title.c_str(), title.size() + 1);
    return;
  }
  ...
  const size_t titleLength = title.size();
  while (sourceOffset < titleLength && outputLength + sizeof(ellipsis) <= sizeof(buffer)) {
    const unsigned char lead = static_cast<unsigned char>(title[sourceOffset]);
    size_t codepointLength = 1;
    if ((lead & 0xE0U) == 0xC0U) codepointLength = 2;
    if ((lead & 0xF0U) == 0xE0U) codepointLength = 3;
    if ((lead & 0xF8U) == 0xF0U) codepointLength = 4;
    if (sourceOffset + codepointLength > titleLength || outputLength + codepointLength + sizeof(ellipsis) > sizeof(buffer)) {
      break;
    }

    const size_t previousLength = outputLength;
    std::memcpy(buffer + outputLength, title.data() + sourceOffset, codepointLength);
    outputLength += codepointLength;
    buffer[outputLength] = '\0';
    if (renderer.getTextWidth(SMALL_FONT_ID, buffer) + ellipsisWidth > maxWidth) {
      outputLength = previousLength;
      buffer[outputLength] = '\0';
      break;
    }
    sourceOffset += codepointLength;
  }

  std::memcpy(buffer + outputLength, ellipsis, sizeof(ellipsis));
}
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
                              ...
                              const bool fillMargin, const bool isPageBookmarked, const int estimatedMinutesLeft,
                              const float* chapterTicksProgress, const int chapterTicksCount) const {
  auto metrics = UITheme::getInstance().getMetrics();
  int orientedMarginTop, orientedMarginRight, orientedMarginBottom, orientedMarginLeft;
  renderer.getOrientedViewableTRBL(&orientedMarginTop, &orientedMarginRight, &orientedMarginBottom,
                                   &orientedMarginLeft);
  if (SETTINGS.statusBarCompactInfo) {
    drawCompactInfoBar(renderer, bookProgress, currentPage, pageCount, title, paddingBottom, orientedMarginBottom,
                       orientedMarginLeft, orientedMarginRight, estimatedMinutesLeft, chapterTicksProgress,
                       chapterTicksCount);
    return;
  }
...
void BaseTheme::drawCompactInfoBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
                                   ...
                                   const int orientedMarginRight, const int estimatedMinutesLeft,
                                   const float* chapterTicksProgress, const int chapterTicksCount) const {
  const auto& metrics = UITheme::getInstance().getMetrics();
  const int screenHeight = renderer.getScreenHeight();
  const int screenWidth = renderer.getScreenWidth();
  const int progressBarY = screenHeight - orientedMarginBottom - paddingBottom - 4;
  const int progressBarWidth = screenWidth - orientedMarginLeft - orientedMarginRight;
  constexpr int progressBarHeight = 4;
  const int filledWidth = static_cast<int>((progressBarWidth * bookProgress) / 100.0f);
  if (filledWidth > 0) {
    renderer.fillRect(orientedMarginLeft, progressBarY, filledWidth, progressBarHeight, true);
  }

  if (chapterTicksProgress != nullptr && chapterTicksCount > 0 && chapterTicksCount <= 20) {
    for (int i = 0; i < chapterTicksCount; ++i) {
      int tickX = orientedMarginLeft + static_cast<int>(progressBarWidth * chapterTicksProgress[i] / 100.0f);
      tickX = std::clamp(tickX, orientedMarginLeft, orientedMarginLeft + progressBarWidth - 1);
      renderer.drawLine(tickX, progressBarY, tickX, progressBarY + progressBarHeight - 1, 1, true);
    }
  }

  const int infoLineY = progressBarY - 20;
  const int outerLeft = metrics.statusBarHorizontalMargin + orientedMarginLeft;
  const int outerRight = metrics.statusBarHorizontalMargin + orientedMarginRight;
  char clockStr[8] = "--:--";
  ...
  if (timestamp > 0) {
    ...
    if (localtime_r(&currentTime, &localTime) != nullptr) {
      snprintf(clockStr, sizeof(clockStr), "%02d:%02d", localTime.tm_hour, localTime.tm_min);
    }
  }

  const int pagesLeft = std::max(0, pageCount - currentPage);
  char timeAndPagesStr[24] = "";
  if (estimatedMinutesLeft >= 0 && pagesLeft > 0) {
    snprintf(timeAndPagesStr, sizeof(timeAndPagesStr), "%dm \xC2\xB7 %dp", estimatedMinutesLeft, pagesLeft);
  } else if (estimatedMinutesLeft >= 0) {
    snprintf(timeAndPagesStr, sizeof(timeAndPagesStr), "%dm", estimatedMinutesLeft);
  } else if (pagesLeft > 0) {
    snprintf(timeAndPagesStr, sizeof(timeAndPagesStr), "--m \xC2\xB7 %dp", pagesLeft);
  }

  const int clockWidth = renderer.getTextWidth(SMALL_FONT_ID, clockStr);
  const bool hasReadout = timeAndPagesStr[0] != '\0';
  const int readoutWidth = hasReadout ? renderer.getTextWidth(SMALL_FONT_ID, timeAndPagesStr) : 0;
  const bool showBatteryPercentage =
      SETTINGS.statusBarBattery && SETTINGS.hideBatteryPercentage == CrossPointSettings::HIDE_BATTERY_PERCENTAGE::HIDE_NEVER;
  char batteryPercentStr[5] = "";
  int batteryTextWidth = 0;
  if (showBatteryPercentage) {
    snprintf(batteryPercentStr, sizeof(batteryPercentStr), "%u%%", static_cast<unsigned>(powerManager.getBatteryPercentage()));
    batteryTextWidth = renderer.getTextWidth(SMALL_FONT_ID, batteryPercentStr);
  }
  const int batterySlotWidth = SETTINGS.statusBarBattery
                                   ? metrics.batteryWidth + (showBatteryPercentage ? batteryPercentSpacing + batteryTextWidth : 0)
                                   : 0;

  auto buildLayout = [&](const bool showClock, const bool showReadout) {
    CompactInfoBarLayout layout;
    layout.showClock = showClock;
    layout.showReadout = showReadout && hasReadout;
    layout.clockX = outerLeft;
    layout.batteryX = screenWidth - outerRight - batterySlotWidth;
    layout.readoutX = layout.showReadout ? layout.batteryX - compactInfoSlotGap - readoutWidth : 0;
    layout.titleX = outerLeft + (layout.showClock ? clockWidth + compactInfoSlotGap : 0);
    const int titleRightEdge = layout.showReadout ? layout.readoutX - compactInfoSlotGap
                                                  : layout.batteryX - compactInfoSlotGap;
    layout.titleWidth = std::max(0, titleRightEdge - layout.titleX);
    return layout;
  };

  CompactInfoBarLayout layout = buildLayout(true, true);
  if (layout.titleWidth < compactInfoMinTitleWidth && layout.showClock) layout = buildLayout(false, true);
  if (layout.titleWidth < compactInfoMinTitleWidth && layout.showReadout) {
    layout = buildLayout(layout.showClock, false);
  }

  if (SETTINGS.statusBarBattery) {
    GUI.drawBatteryLeft(renderer, Rect{layout.batteryX, infoLineY + 6, metrics.batteryWidth, metrics.batteryHeight},
                        showBatteryPercentage);
  }
  if (layout.showReadout) renderer.drawText(SMALL_FONT_ID, layout.readoutX, infoLineY, timeAndPagesStr);
  if (layout.showClock) renderer.drawText(SMALL_FONT_ID, layout.clockX, infoLineY, clockStr);
  if (!title.empty() && layout.titleWidth > 0) {
    char titleBuffer[compactInfoTitleBufferSize];
    truncateCompactInfoTitle(renderer, title, layout.titleWidth, titleBuffer);
    const int titleWidth = renderer.getTextWidth(SMALL_FONT_ID, titleBuffer);
    renderer.drawText(SMALL_FONT_ID, layout.titleX + std::max(0, (layout.titleWidth - titleWidth) / 2), infoLineY,
                      titleBuffer);
  }
}
