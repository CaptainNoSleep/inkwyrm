...
void stampSuccessfulProgressSync() {
  const uint32_t timestamp = TimeUtils::getCurrentValidTimestamp();
  if (timestamp == 0) {
    LOG_DBG("KOSync", "Progress sync timestamp skipped because clock is not valid");
    return;
  }
  SETTINGS.lastKoSyncEpoch = timestamp;
  SETTINGS.saveToFile();
}
}  // namespace
...
void KOReaderSyncActivity::saveProgressAndReturn(int spineIndex, int page) {
  ...
  if (!EpubReaderUtils::saveProgress(*epub, spineIndex, page, 0)) {
    if (launchMode == KOReaderSyncLaunchMode::AUTO_PULL) {
      LOG_DBG("KOSync", "Auto-pull could not persist remote position; keeping local position");
      returnToReader();
      return;
    }
    {
  ...
  stampSuccessfulProgressSync();
  returnToReader();
}

void KOReaderSyncActivity::returnToReader() { activityManager.goToReader(epubPath); }

void KOReaderSyncActivity::returnToHome() { activityManager.goHome(); }

void KOReaderSyncActivity::onWifiSelectionComplete(const bool success) {
  if (!success) {
    LOG_DBG("KOSync", "WiFi connection failed, exiting %s sync silently",
            isAutomatic() ? "automatic" : "manual");
    if (launchMode == KOReaderSyncLaunchMode::AUTO_PUSH) {
      returnToHome();
    } else {
      returnToReader();
    }
...
void KOReaderSyncActivity::performSync() {
  ...
  if (documentHash.empty()) {
    if (isAutomatic()) {
      LOG_DBG("KOSync", "Automatic sync could not calculate document hash; keeping local position");
      if (launchMode == KOReaderSyncLaunchMode::AUTO_PUSH) {
        returnToHome();
      } else {
        returnToReader();
  ...
  if (result == KOReaderSyncClient::NOT_FOUND) {
    if (launchMode == KOReaderSyncLaunchMode::AUTO_PULL) {
      LOG_DBG("KOSync", "Auto-pull found no remote progress; keeping local position");
      returnToReader();
      return;
    }
  ...
  if (result != KOReaderSyncClient::OK) {
    if (isAutomatic()) {
      LOG_DBG("KOSync", "Automatic sync failed (%d); keeping local position", result);
      if (launchMode == KOReaderSyncLaunchMode::AUTO_PUSH) {
        returnToHome();
      } else {
        returnToReader();
  ...
  if (!epub) {
    if (isAutomatic()) {
      LOG_DBG("KOSync", "Automatic sync could not reload EPUB; keeping local position");
      if (launchMode == KOReaderSyncLaunchMode::AUTO_PUSH) {
        returnToHome();
      } else {
        returnToReader();
  ...
  if (launchMode == KOReaderSyncLaunchMode::AUTO_PULL) {
    // Match master's conservative rule: do not replace a local position unless
    // the remote percentage is meaningfully ahead.
    if (remoteProgress.percentage > localProgress.percentage + 0.0005f) {
      LOG_DBG("KOSync", "Auto-pull applying newer remote position: %.4f > %.4f", remoteProgress.percentage,
              localProgress.percentage);
      saveProgressAndReturn(remotePosition.spineIndex, remotePosition.pageNumber);
    } else {
      LOG_DBG("KOSync", "Auto-pull keeping local position: remote=%.4f local=%.4f", remoteProgress.percentage,
              localProgress.percentage);
      returnToReader();
    }
    return;
...
void KOReaderSyncActivity::performUpload() {
  ...
  if (result != KOReaderSyncClient::OK) {
    if (launchMode == KOReaderSyncLaunchMode::AUTO_PUSH) {
      LOG_DBG("KOSync", "Auto-push failed (%d); leaving reader silently", result);
      returnToHome();
      return;
    }
    {
...
void KOReaderSyncActivity::onEnter() {
  ...
  startActivityForResult(std::make_unique<WifiSelectionActivity>(renderer, mappedInput, true, isAutomatic()),
