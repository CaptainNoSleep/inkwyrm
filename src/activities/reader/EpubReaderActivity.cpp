...
void EpubReaderActivity::onEnter() {
  ...
  highlightStore.load(epub->getCachePath(), stableBookId);
  ...
  // The sync activity must restart to reclaim WiFi/TLS heap. Consume its
  // one-shot marker here so that the resumed reader does not auto-pull again.
  if (APP_STATE.koSyncAutoPullResumePath == epub->getPath()) {
    APP_STATE.koSyncAutoPullResumePath.clear();
    APP_STATE.saveToFile();
  } else if (SETTINGS.koSyncAutoPullOnOpen && KOREADER_STORE.hasCredentials()) {
    if (launchKOReaderSync(SyncLaunchMode::AUTO_PULL)) {
      return;
    }
  }
...
void EpubReaderActivity::loop() {
  ...
  if (atEndOfBook) {
    ...
    // Stats: reaching the End-of-Book screen marks the book completed. updateProgress is
    // change-guarded internally, so repeated loop() calls after the transition are no-ops.
  ...
  // A single power tap is inert by default while awake; use two press edges to
  // toggle reader-only dark mode without delaying any existing short-tap action.
  if (mappedInput.wasPressed(MappedInputManager::Button::Power)) {
    const unsigned long now = millis();
    // Swallow the post-toggle burst so one gesture can't flip dark mode repeatedly.
    if (lastDarkToggleTime != 0UL && now - lastDarkToggleTime <= DARK_TOGGLE_COOLDOWN_MS) {
      lastPowerTapTime = 0UL;
      return;
    }
    if (lastPowerTapTime != 0UL && now - lastPowerTapTime <= POWER_DOUBLE_TAP_WINDOW_MS) {
      SETTINGS.readerDarkMode = !SETTINGS.readerDarkMode;
      SETTINGS.saveToFile();
      lastPowerTapTime = 0UL;
      lastDarkToggleTime = now;
      forceFullRefreshAfterDarkModeToggle = true;
      requestUpdate();
      return;
    }
    lastPowerTapTime = now;
  }
...
void EpubReaderActivity::drawHighlightUnderline(const HighlightGeometry::WordInfo& word) const {
  const int underlineY = word.screenY + renderer.getLineHeight(SETTINGS.getReaderFontId()) - 2;
  renderer.drawLine(word.screenX, underlineY, word.screenX + word.width - 1, underlineY, true);
}

void EpubReaderActivity::renderCurrentPageHighlights(const Page& page, const int orientedMarginLeft,
                                                     const int orientedMarginTop) {
  if (highlightStore.isEmpty() || !section) return;
  const auto& records = highlightStore.getAll();
  // Extraction + font prewarm are expensive (per-word string copies); skip them
  // entirely unless the CURRENT page actually has a highlight.
  bool pageHasHighlight = false;
  for (const auto& record : records) {
    if (record.spineIndex == static_cast<uint16_t>(currentSpineIndex) &&
        record.pageNumber == static_cast<uint16_t>(section->currentPage)) {
      pageHasHighlight = true;
      break;
    }
  }
  if (!pageHasHighlight) return;
  std::vector<HighlightGeometry::WordInfo> words;
  std::vector<HighlightGeometry::RowInfo> rows;
  words.reserve(128);
  rows.reserve(32);
  HighlightGeometry::extractWords(page, renderer, SETTINGS.getReaderFontId(), orientedMarginLeft, orientedMarginTop,
                                  words, rows);
  for (const auto& record : records) {
    if (record.spineIndex != static_cast<uint16_t>(currentSpineIndex) ||
        record.pageNumber != static_cast<uint16_t>(section->currentPage)) continue;
    const int lo = std::min(record.anchorWordIndex, record.focusWordIndex);
    const int hi = std::max(record.anchorWordIndex, record.focusWordIndex);
    if (lo < 0 || hi < 0 || lo >= static_cast<int>(words.size()) || hi >= static_cast<int>(words.size())) continue;
    for (int i = lo; i <= hi; ++i) drawHighlightUnderline(words[static_cast<size_t>(i)]);
  }
}

void EpubReaderActivity::openHighlightSelection() {
  if (!section || section->currentPage < 0 || section->currentPage >= section->pageCount) {
    requestUpdate();
    return;
  }
  ...
  READING_STATS.noteActivity();
  startActivityForResult(std::make_unique<HighlightSelectionActivity>(
      renderer, mappedInput, page, SETTINGS.getReaderFontId(), orientedMarginLeft, orientedMarginTop,
      currentSpineIndex, section->currentPage), [this](const ActivityResult& result) {
    READING_STATS.resumeSession();
    if (!result.isCancelled) {
      const auto* highlightPtr = std::get_if<HighlightResult>(&result.data);
      if (!highlightPtr) { requestUpdate(); return; }
      const auto& highlight = *highlightPtr;
      if (highlightStore.hasDuplicate(highlight)) {
        GUI.drawPopup(renderer, tr(STR_HIGHLIGHT_ALREADY_SAVED));
      } else if (highlightStore.add(highlight)) {
        highlightStore.save();
...
void EpubReaderActivity::openHighlightList() {
  READING_STATS.noteActivity();
  startActivityForResult(std::make_unique<HighlightsActivity>(
      renderer, mappedInput, highlightStore.getAll(), epub, tr(STR_HIGHLIGHTS),
      [this](const HighlightStore::HighlightEntry& highlight) {
        const bool removed = highlightStore.removeById(highlight.id);
        if (removed) highlightStore.save();
        return removed;
      }), [this](const ActivityResult& result) {
    READING_STATS.resumeSession();
    if (!result.isCancelled) {
      const auto* bookmarkPtr = std::get_if<BookmarkResult>(&result.data);
      if (!bookmarkPtr) { requestUpdate(); return; }
      const auto& bookmark = *bookmarkPtr;
...
bool EpubReaderActivity::launchKOReaderSync(const SyncLaunchMode mode) {
  ...
  if (!saveProgress(currentSpineIndex, currentPage, totalPages)) {
    ...
    if (mode == SyncLaunchMode::AUTO_PULL || mode == SyncLaunchMode::AUTO_PUSH) {
      return false;
    }
  ...
  if (mode == SyncLaunchMode::AUTO_PULL) {
    APP_STATE.koSyncAutoPullResumePath = savedEpubPath;
    // The persisted marker is the loop-breaker for the post-sync restart. If it
    // cannot be persisted, skip auto-pull entirely (manual sync still works)
    // rather than risk a pull->restart->pull cycle on a failing SD.
    if (!APP_STATE.saveToFile()) {
      APP_STATE.koSyncAutoPullResumePath.clear();
      LOG_ERR("KOSync", "Skipping auto-pull: one-shot marker could not be persisted");
      return false;
    }
  }
  ...
  activityManager.replaceActivity(std::make_unique<KOReaderSyncActivity>(
      ...
      mode == SyncLaunchMode::AUTO_PULL   ? KOReaderSyncLaunchMode::AUTO_PULL
      : mode == SyncLaunchMode::AUTO_PUSH ? KOReaderSyncLaunchMode::AUTO_PUSH
                                          : KOReaderSyncLaunchMode::COMPARE));
...
bool EpubReaderActivity::tryAutoPushOnClose() {
  ...
  return launchKOReaderSync(SyncLaunchMode::AUTO_PUSH);
}
...
bool EpubReaderActivity::saveProgress(int spineIndex, int currentPage, int pageCount) {
  if (spineIndex != sessionStartSpineIndex || currentPage != sessionStartPage) {
...
void EpubReaderActivity::renderContents(std::unique_ptr<Page> page, const int orientedMarginTop,
  ...
  // Dark mode applies to text pages (not image pages yet — inverting images
  // would render photo negatives; that waits on the whole-theme work). When
  // dark mode is active we render text in plain BW and skip anti-aliasing:
  // the grayscale AA planes are drawn separately after the BW base and can't be
  // inverted here, so BW text (which invertScreen flips cleanly to white-on-
  // black) is what makes dark mode actually engage instead of silently gating
  // off whenever AA is on (the default). Text is crisp, just not smoothed, in
  // dark mode — a fine trade for night reading.
  const bool renderDarkMode = SETTINGS.readerDarkMode && !pageHasImages;
  const bool needsTextGrayscale = SETTINGS.textAntiAliasing && !renderDarkMode;
  ...
  page->render(renderer, fontId, orientedMarginLeft, orientedMarginTop);
  renderCurrentPageHighlights(*page, orientedMarginLeft, orientedMarginTop);
  renderStatusBar();
  if (renderDarkMode) {
  ...
  // A FULL refresh (which briefly flashes the panel) is only warranted when the
  // rendered polarity actually flips. On a page where dark mode can't apply
  // (images), toggling changes nothing on screen, so we must NOT force a
  // refresh — that was the "flash then stays light" glitch. Drive the refresh
  // purely off the real polarity transition.
  const int8_t polarity = renderDarkMode ? 1 : 0;
  const bool polarityChanged = lastRenderedPolarity >= 0 && polarity != lastRenderedPolarity;
  lastRenderedPolarity = polarity;
  forceFullRefreshAfterDarkModeToggle = false;
  if (polarityChanged) {
...
void EpubReaderActivity::renderStatusBar() const {
  ...
  // Calculate time estimate from reading stats (for compact info bar)
  int estimatedMinutesLeft = -1;
  if (SETTINGS.statusBarCompactInfo && epub) {
    const auto* bookStats = READING_STATS.findBook(epub->getPath());
    if (bookStats && bookStats->totalReadingMs > 0 && bookStats->lastProgressPercent > 0) {
      // Calculate minutes per percentage point based on historical data
      const float minutesPerPercent =
          static_cast<float>(bookStats->totalReadingMs) / static_cast<float>(bookStats->lastProgressPercent) / 60000.0f;
      // Estimate remaining time for book
      const float remainingPercent = 100.0f - bookProgress;
      if (remainingPercent > 0) {
        const float estimateFloat = remainingPercent * minutesPerPercent + 0.5f;
        // Clamp to prevent overflow (corrupt/huge totalReadingMs can push outside int range)
        estimatedMinutesLeft = (estimateFloat > 9999.0f) ? 9999 : static_cast<int>(estimateFloat);
      }
    }
  }

  // Calculate chapter tick positions (for compact info bar)
  constexpr int MAX_CHAPTER_TICKS = 20;
  float chapterTicksProgress[MAX_CHAPTER_TICKS];
  int chapterTicksCount = 0;
  if (SETTINGS.statusBarCompactInfo && epub) {
    const int tocCount = epub->getTocItemsCount();
    // Cap at 20 to avoid clutter on 480px screen
    if (tocCount > 0 && tocCount <= MAX_CHAPTER_TICKS) {
      for (int i = 0; i < tocCount; ++i) {
        const int spineIndex = epub->getSpineIndexForTocIndex(i);
        if (spineIndex >= 0) {
          const float chapterProgress = epub->calculateProgress(spineIndex, 0.0f) * 100.0f;
          chapterTicksProgress[chapterTicksCount++] = chapterProgress;
        }
      }
    }
  }
  ...
  GUI.drawStatusBar(renderer, bookProgress, currentPage, pageCount, title, 0, textYOffset, true, currentPageBookmarked,
                    estimatedMinutesLeft, chapterTicksCount > 0 ? chapterTicksProgress : nullptr, chapterTicksCount);
}
