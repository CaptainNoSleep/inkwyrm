...
void WifiSelectionActivity::attemptConnection(const bool isRetry) {
  if (!isRetry) {
    connectionRetryAttempted = false;
  }
  ...
  // ESP32-C3: the default modem power-save (WIFI_PS_MIN_MODEM) can nap through
  // auth/4-way-handshake frames and stall the join with NO error event
  // (upstream crosspoint PR #2582 diagnosis). Must run AFTER begin(): the
  // driver restart inside begin() re-enables the default power-save mode.
  WiFi.setSleep(false);

  // Full TX power on every attempt. The 8.5dBm "brownout" workaround was tried
  // and reverted: device logs show association TIMEOUTS with no reset, i.e. a
  // weak AP<->device link, not a brownout. Lowering TX makes the device harder
  // for the AP to hear during association, so it hurt this symptom. Keep max
  // range for the join; brownout (which shows as a reset under load) is not what
  // this hardware exhibits here.
  WiFi.setTxPower(WIFI_POWER_19_5dBm);
}

bool WifiSelectionActivity::retryConnection() {
  if (connectionRetryAttempted) {
    return false;
  }

  connectionRetryAttempted = true;
  LOG_DBG("WIFI", "Retrying WiFi join once");
...
void WifiSelectionActivity::checkConnectionStatus() {
  ...
  if (status == WL_CONNECT_FAILED || status == WL_NO_SSID_AVAIL) {
    ...
    if (autoConnectOnly) {
      LOG_DBG("WIFI", "Automatic connection failed; returning without WiFi picker");
      onComplete(false);
      return;
    }
  ...
  // Check for timeout. Background auto-sync (autoConnectOnly) uses a SHORT
  // timeout and NO retry: opening a book must never hang for tens of seconds
  // waiting on WiFi. A healthy join associates in 2-5s (the old 25s was sized
  // for the broken AP that RSSI-kicked us). If we're not on in AUTO_CONNECT_MS,
  // we're effectively offline — bail and open the book. Manual sync (the user
  // explicitly asked) keeps the full timeout + one retry.
  const unsigned long timeoutMs = autoConnectOnly ? AUTO_CONNECT_TIMEOUT_MS : CONNECTION_TIMEOUT_MS;
  if (millis() - connectionStartTime > timeoutMs) {
    WiFi.disconnect();
    connectionError = tr(STR_ERROR_CONNECTION_TIMEOUT);
    if (!autoConnectOnly && retryConnection()) {
      return;
    }
    if (autoConnectOnly) {
      LOG_DBG("WIFI", "Auto-connect timed out (%lums); opening book without sync", timeoutMs);
      onComplete(false);
      return;
    }
    state = WifiSelectionState::CONNECTION_FAILED;
