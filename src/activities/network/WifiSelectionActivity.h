...
class WifiSelectionActivity final : public Activity {
  ...
  // Auto-sync must never fall through to the interactive WiFi picker.
  ...
  static constexpr unsigned long AUTO_CONNECT_TIMEOUT_MS = 8000;  // background auto-sync: fail fast, never hang book-open
  bool connectionRetryAttempted = false;
  ...
  void attemptConnection(bool isRetry = false);
